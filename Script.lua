...
--[[
    ...
    to keep pressure on the map without player input.  By default the script enables
    a "maximum difficulty" preset so the controller is tuned for the most demanding
    gameplay UCZone can provide right after loading the script.
...
local ui = {
    enabled = general_group:Switch("Enable OpenHyperAI autopilot", false),
...
--#region helpers
local function apply_maximum_difficulty_defaults()
    if ui.enabled and ui.enabled.Set then
        ui.enabled:Set(true)
    end

    if ui.target_radius and ui.target_radius.Set then
        ui.target_radius:Set(1800)
    end

    if ui.focus_low_hp and ui.focus_low_hp.Set then
        ui.focus_low_hp:Set(350)
    end

    if ui.combo_window and ui.combo_window.Set then
        ui.combo_window:Set(1.5)
    end

    if ui.panic_threshold and ui.panic_threshold.Set then
        ui.panic_threshold:Set(30)
    end

    if ui.retreat_threshold and ui.retreat_threshold.Set then
        ui.retreat_threshold:Set(22)
    end

    if ui.move_interval and ui.move_interval.Set then
        ui.move_interval:Set(200)
    end

    if ui.unit_update_interval and ui.unit_update_interval.Set then
        ui.unit_update_interval:Set(400)
    end
end

apply_maximum_difficulty_defaults()
